...
T_test = trials_test * pulse_interval

# set up input and output (different from tutorial)
with nengo.Network(seed=seed) as model:
    in_u = nengo.Node(output=nengo.processes.PresentInput(u_train, dt))
    z_out = nengo.Node(output=nengo.processes.PresentInput(y_train, dt))
    
# Initial weights
e_in = g_in * rng.uniform(-1, +1, (n, u_train.shape[1]))  # fixed encoders for f_in (u_in)
...
##### ----- train readout ----- #####
train_target = sim.data[p_target]
...
rr_model = Ridge()
rr_model.fit(sim.data[p_readout], train_target)
train_pred = sim.data[p_readout].dot(rr_model.coef_.T)
...
plt.plot(sim.trange(), train_pred, label="prediction")
plt.plot(sim.trange(), train_target, label="target")
...
##### ----- evaluation ----- #####
# set up input and output (different from tutorial)
with nengo.Network(seed=seed) as new_model:
    in_u_test = nengo.Node(output=nengo.processes.PresentInput(u_test, dt))
    z_out_test = nengo.Node(output=nengo.processes.PresentInput(y_test, dt))
    
# Initial weights
e_in_test = g_in * rng.uniform(-1, +1, (n, u_test.shape[1]))  # fixed encoders for f_in (u_in)
# e_out = g_out * rng.uniform(-1, +1, (n, 1))  # fixed encoders for f_out (u)
res_weights_test = rng.randn(n, n) * g / np.sqrt(n)  # target-generating weights (variance g^2/n) (weights for reservoir)

# model
with new_model:
    xD_test = nengo.Ensemble(**ens_kwargs) # reservoir
    sD_test = nengo.Node(size_in=n)  # pre filter
    
    nengo.Connection(in_u_test, sD_test, synapse=None, transform=e_in_test) # filter input
    nengo.Connection(sD_test, xD_test.neurons, synapse=tau) # filtered input to reservoir
    nengo.Connection(xD_test.neurons, sD_test, synapse=None, transform=res_weights_test)  # chaos
    
# probes
with new_model:
    p_readout_test = nengo.Probe(xD_test.neurons, synapse=tau_probe)
    p_target_test = nengo.Probe(z_out_test, synapse=tau_probe)
    
# run simulation
with nengo.Simulator(new_model, dt=dt) as sim_test:
    sim_test.run(T_test)

test_target = sim_test.data[p_target_test]
test_pred = sim_test.data[p_readout_test].dot(rr_model.coef_.T)
...
plt.plot(sim_test.trange(), test_pred, label="prediction")
plt.plot(sim_test.trange(), test_target, label="target")
